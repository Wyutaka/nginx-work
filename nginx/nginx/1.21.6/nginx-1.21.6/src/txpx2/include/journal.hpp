...
constexpr unsigned long long operator"" _B(unsigned long long l) noexcept {
  return l;
}
...
class MmapPool {
 ...
 private:
  void allocateNew() {
    ...
    auto file_name =
        base_directory_ + "/journal." + std::to_string(allocated_++);
    ...
    auto ptr = ::mmap(nullptr, allocate_size_, PROT_WRITE | PROT_READ,
                      MAP_SHARED, fd, 0);
    syscall_assert(ptr != MAP_FAILED, mmap);
    std::clog << allocate_size_ << " Bytes of journal area created on "
              << file_name << std::endl;
 ...
 public:
  explicit MmapJournal(std::shared_ptr<MmapPool> pool, bool async_commit)
      : pool_(std::move(pool)),
        ...
        last_(reinterpret_cast<char *>(
            std::get<MmapPool::kPointerIndex>(area_))) {}

  ~MmapJournal() { pool_->deallocate(area_); }

 public:
  void append(const boost::string_view &str) {
  ...
  std::vector<boost::string_view> get() const {
    auto first = reinterpret_cast<const char *>(
        std::get<MmapPool::kPointerIndex>(area_));

    std::vector<boost::string_view> contents;
